...
def online(**hyperparams):
    run = wandb.init(project="transformer-style-transfer", entity="jukamala", reinit=True)
...
def sweep(count=10, name="BERT", metric="val_MSE", lr=True, lr_st=False, **hyperparams):
    ...
    sweep_id = wandb.sweep(sweep_config, project="transformer-style-transfer", entity="jukamala")
...
def existing(count=5):
    ...
    wandb.agent(sweep_id, function=train, count=count, project="transformer-style-transfer", entity="jukamala")


if __name__ == "__main__":

    tasks = ["gp-train-iter", "gp-train-iter_hc", "gp-train-mft", "gp-train-mat", "gp-train-malstm", "gp-train-macnn"]
